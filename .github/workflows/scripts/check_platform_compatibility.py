...
class PlatformCompatibilityChecker:
    ...
    def find_files(self, extensions=None):
        ...
        for ext in extensions:
            for file in glob.glob(f"**/*{ext}", recursive=True):
                ...
                if any(path in file for path in ['.git/', 'venv/', '.github/workflows/']):
                    continue
                files.append(file)
    ...
    def check_platform_specific_imports(self, file_path):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                platform_imports = {
                    "windows": ["winreg", "_winreg", "msvcrt", "win32api", "win32con", "win32com", "win32gui"],
                ...
                lines = content.split('\n')
                for i, line in enumerate(lines):
                    if not line.strip().startswith('#') and re.search(r'^\s*(?:import|from)\s+', line):
                        for platform, imports in platform_imports.items():
                            for import_name in imports:
                                if re.search(rf'(?:import|from)\s+{import_name}(?:\s+|\.|$)', line):
                                    issues.append({
                                        "file": file_path,
                                        "line": i + 1,
    ...
    def generate_report(self, all_issues):
        ...
        markdown_path = os.path.join(self.report_dir, "platform_compatibility_report.md")
        with open(markdown_path, 'w', encoding='utf-8') as f:
            f.write("# Platform Compatibility Report\n\n")
            f.write("This report identifies potential platform-specific code that may cause compatibility issues.\n\n")
            
            # Summary section
            f.write("## Summary\n\n")
            f.write(f"- **Total files analyzed**: {len(issues_by_file)}\n")
            f.write(f"- **Total issues found**: {len(all_issues)}\n")
            f.write(f"- **Error level issues**: {len(issues_by_severity.get('error', []))}\n")
            f.write(f"- **Warning level issues**: {len(issues_by_severity.get('warning', []))}\n")
            f.write(f"- **Info level issues**: {len(issues_by_severity.get('info', []))}\n\n")
            ...
            f.write("|------|--------------|--------|----------|------|\n")
            for file, issues in sorted(issues_by_file.items(), key=lambda x: len(x[1]), reverse=True):
                errors = sum(1 for i in issues if i["severity"] == "error")
                warnings = sum(1 for i in issues if i["severity"] == "warning")
                infos = sum(1 for i in issues if i["severity"] == "info")
                f.write(f"| {file} | {len(issues)} | {errors} | {warnings} | {infos} |\n")
            f.write("\n")
            ...
            for file, issues in sorted(issues_by_file.items()):
                ...
                for issue in sorted(issues, key=lambda x: (x["severity"] != "error", x["severity"] != "warning", x["line"])):
                    severity_emoji = "🔴" if issue["severity"] == "error" else "⚠️" if issue["severity"] == "warning" else "ℹ️"
                    platforms = ", ".join(p.capitalize() for p in issue.get("platforms", []))
                    line_info = f"Line {issue['line']}: " if issue['line'] else ""
                    
                    f.write(f"- {severity_emoji} **{issue['pattern_name']}** ({platforms}) - {line_info}{issue['description']}\n")
                
                f.write("\n")
            
            # Recommendations
            f.write("## Recommendations\n\n")
            f.write("1. **Use os.path for file operations**: Replace hardcoded path separators with `os.path.join()`\n")
            f.write("2. **Use pathlib for modern path handling**: Consider using the cross-platform `pathlib` module\n")
            f.write("3. **Check platform conditionally**: Use `if sys.platform == 'win32'` for platform-specific code\n")
            f.write("4. **Use consistent line endings**: Configure your editor to use LF (Unix-style) line endings\n")
            f.write("5. **Abstract platform-specific operations**: Create utility functions to abstract OS-specific code\n\n")
            
            f.write("## Next Steps\n\n")
            f.write("- Review error-level issues first as they're most likely to cause compatibility problems\n")
            f.write("- Consider adding cross-platform tests to verify behavior across different operating systems\n")
            f.write("- Use techniques like dependency injection to make platform-specific code more testable\n\n")
            
            # Footer
            f.write("---\n\n")
            f.write(f"*This report was automatically generated by the CI process on {os.popen('date').read().strip()}*\n")
    ...
    def generate_github_summary(self, all_issues):
        ...
        with open(os.environ['GITHUB_STEP_SUMMARY'], 'a', encoding='utf-8') as f:
            ...
            f.write("### Overview\n\n")
            f.write(f"Found **{len(all_issues)}** potential platform compatibility issues\n\n")
            
            # Status indicators
            if error_count > 0:
                f.write("🔴 **Platform-specific issues found that may break cross-platform compatibility**\n\n")
            elif warning_count > 0:
                f.write("⚠️ **Minor platform-specific issues found that should be reviewed**\n\n")
            else:
            ...
            if error_count > 0:
                f.write("⚠️ **Recommendation**: Review error-level issues immediately to ensure cross-platform compatibility\n")
            elif warning_count > 0:
                f.write("ℹ️ **Recommendation**: Consider refactoring code with platform-specific patterns for better compatibility\n")
            else:
                f.write("✅ **Recommendation**: Continue maintaining good cross-platform practices\n")
    ...
    def run(self):
        ...
        print(f"Platform compatibility analysis complete. Reports saved to {markdown_path} and {json_path}")
        
        # Return exit code based on errors
        return 1 if any(issue["severity"] == "error" for issue in all_issues) else 0

def main():
    parser = argparse.ArgumentParser(description="Check for platform-specific issues in the codebase")
    parser.add_argument("--report-dir", default="ci_artifacts/platform_checks", help="Directory to store the report")
