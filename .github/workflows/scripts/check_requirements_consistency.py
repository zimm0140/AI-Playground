...
class RequirementsChecker:
    ...
    def analyze_file(self, file_path):
        ...
        if not os.path.exists(file_path):
            return {
                "file": file_path,
                "errors": [{"rule": "file_existence", "message": "File does not exist"}],
                "warnings": [],
                "info": [],
                "metrics": {}
            }
        ...
        for rule in self.rules:
            ...
            if rule["name"] == "header_comment":
                ...
                if result["metrics"]["package_lines"] > 5 and result["metrics"]["section_markers"] == 0:
                    result[self._get_severity_list(severity)].append({
                        "rule": rule["name"],
                ...
                for i, line in enumerate(lines):
                    if line.startswith('--extra-index-url') and not re.match(pattern, line):
                        result[self._get_severity_list(severity)].append({
                            "rule": rule["name"],
    ...
    def _check_version_consistency(self, lines, result):
        ...
        if len(version_formats) > 1 and min(version_formats.values()) > 0:
            formats = ", ".join([f"{fmt} ({count} times)" for fmt, count in version_formats.items()])
    ...
    def generate_markdown_report(self, results):
        ...
        report_path = os.path.join(self.report_dir, "requirements_consistency_report.md")
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            # Summary table
            f.write("| File | Packages | Pinned Versions | Unpinned | Comments | Sections | Errors | Warnings |\n")
            f.write("|------|----------|----------------|----------|----------|----------|--------|----------|\n")
            
            for result in results:
                metrics = result["metrics"]
                f.write(f"| {result['file']} | {metrics.get('package_lines', 0)} | {metrics.get('pinned_versions', 0)} | {metrics.get('unpinned_versions', 0)} | {metrics.get('comment_lines', 0)} | {metrics.get('section_markers', 0)} | {len(result['errors'])} | {len(result['warnings'])} |\n")
            ...
            f.write("1. **Standardize version pinning**: Use the `==` format consistently for all packages.\n")
            f.write("2. **Add section markers**: Use section headers like `# === SECTION ===` to organize requirements.\n")
            f.write("3. **Add header comments**: Each requirements file should have a comment header explaining its purpose.\n")
            f.write("4. **Pin all versions**: Specify exact versions for all packages to ensure reproducibility.\n")
            f.write("5. **Consistent naming**: Follow consistent package naming conventions.\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Check requirements files for consistency")
    parser.add_argument("--report-dir", default="ci_artifacts/requirements_check", help="Directory to store the report")
