...
class CIMetricsCollector:
    ...
    def collect_test_metrics(self):
        ...
        test_result_files = glob.glob(f"{self.artifacts_dir}/*/test_results.json") + \
        ...
        for result_file in test_result_files:
            try:
                ...
                if platform not in self.metrics_data["test_metrics"]["platform_results"]:
                ...
                platform_metrics = self.metrics_data["test_metrics"]["platform_results"][platform]
                ...
                self.metrics_data["test_metrics"]["total_tests"] += results.get("total", 0)
                self.metrics_data["test_metrics"]["passed_tests"] += results.get("passed", 0)
                self.metrics_data["test_metrics"]["failed_tests"] += results.get("failed", 0)
                self.metrics_data["test_metrics"]["skipped_tests"] += results.get("skipped", 0)
                self.metrics_data["test_metrics"]["test_duration"] += results.get("duration", 0)
    ...
    def collect_coverage_metrics(self):
        ...
        coverage_files = glob.glob(f"{self.artifacts_dir}/coverage*/*.xml") + \
                        glob.glob(f"{self.artifacts_dir}/*/coverage.xml")
        ...
        for coverage_file in coverage_files:
            try:
                ...
                self.metrics_data["coverage_metrics"]["platform_coverage"][platform] = coverage_pct
                    ...
                    for package in root.findall('.//package'):
                        ...
                        for cls in package.findall('.//class'):
                            ...
                            if filename:
                                ...
                                self.metrics_data["coverage_metrics"]["file_coverage"][filename] = file_coverage
                
                # Use Linux coverage as the overall coverage if available
                if platform == "linux" and "overall_coverage" in self.metrics_data["coverage_metrics"]:
                    self.metrics_data["coverage_metrics"]["overall_coverage"] = coverage_pct
            
            except Exception as e:
                print(f"Error processing coverage file {coverage_file}: {str(e)}")
        
        # Calculate average coverage if no Linux coverage file was found
        if self.metrics_data["coverage_metrics"]["overall_coverage"] == 0 and self.metrics_data["coverage_metrics"]["platform_coverage"]:
            platform_coverages = list(self.metrics_data["coverage_metrics"]["platform_coverage"].values())
            self.metrics_data["coverage_metrics"]["overall_coverage"] = round(sum(platform_coverages) / len(platform_coverages), 2)
    ...
    def collect_performance_metrics(self):
        ...
        performance_files = glob.glob(f"{self.artifacts_dir}/performance*/performance_data.csv")
            ...
            try:
                with open(perf_file, 'r') as f:
                    ...
                    for row in reader:
                        ...
                        self.metrics_data["performance_metrics"]["total_duration"] += duration
                    
                    # Store step durations
                    self.metrics_data["performance_metrics"]["step_durations"] = step_durations
                    
                    # Find slowest steps
                    sorted_steps = sorted(step_durations.items(), key=lambda x: x[1]["duration"], reverse=True)
        ...
        print(f"Collected performance metrics: {self.metrics_data['performance_metrics']}")
    ...
    def collect_security_metrics(self):
        ...
        for security_file in security_files:
            try:
                ...
                # Update metrics
                self.metrics_data["security_metrics"]["high_vulnerabilities"] += high_matches
                self.metrics_data["security_metrics"]["medium_vulnerabilities"] += medium_matches
                self.metrics_data["security_metrics"]["low_vulnerabilities"] += low_matches
                self.metrics_data["security_metrics"]["total_vulnerabilities"] += (high_matches + medium_matches + low_matches)
                ...
                for line in lines:
                    if "vulnerability found in" in line:
                        ...
                        if len(parts) > 1:
                            ...
                            if package and package not in self.metrics_data["security_metrics"]["vulnerable_packages"]:
                                self.metrics_data["security_metrics"]["vulnerable_packages"].append(package)
    ...
    def collect_platform_metrics(self):
        ...
        platform_files = glob.glob(f"{self.artifacts_dir}/platform*/platform_issues.json")
        ...
        for platform_file in platform_files:
            try:
                ...
                # Count total issues
                self.metrics_data["platform_metrics"]["total_issues"] = len(platform_issues)
                
                # Count by severity
                for issue in platform_issues:
                    severity = issue.get("severity", "info")
                    self.metrics_data["platform_metrics"]["severity_counts"][severity] += 1
                    
                    # Count by platform
                    for platform in issue.get("platforms", []):
                        if platform not in self.metrics_data["platform_metrics"]["platform_issues"]:
                            self.metrics_data["platform_metrics"]["platform_issues"][platform] = 0
                        self.metrics_data["platform_metrics"]["platform_issues"][platform] += 1
    ...
    def generate_metrics_files(self):
        ...
        history_entry = {
            ...
            "overall_coverage": self.metrics_data["coverage_metrics"].get("overall_coverage", 0),
            "total_duration": self.metrics_data["performance_metrics"].get("total_duration", 0),
            "total_vulnerabilities": self.metrics_data["security_metrics"].get("total_vulnerabilities", 0),
            "platform_issues": self.metrics_data["platform_metrics"].get("total_issues", 0)
        }
    ...
    def generate_markdown_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"- **Total Duration**: {test_metrics.get('test_duration', 0):.2f} seconds\n\n")
            
            # Platform Test Results
            if test_metrics.get("platform_results"):
                f.write("### Platform Test Results\n\n")
                f.write("| Platform | Tests | Passed | Failed | Skipped | Pass Rate |\n")
                f.write("|----------|-------|--------|--------|---------|----------|\n")
                
                for platform, metrics in test_metrics.get("platform_results", {}).items():
                    total = metrics.get("total_tests", 0)
                    pass_rate = round((metrics.get("passed_tests", 0) / total) * 100, 2) if total > 0 else 0
                    f.write(f"| {platform.capitalize()} | {total} | {metrics.get('passed_tests', 0)} | {metrics.get('failed_tests', 0)} | {metrics.get('skipped_tests', 0)} | {pass_rate}% |\n")
            ...
            f.write(f"- **Overall Coverage**: {coverage_metrics.get('overall_coverage', 0)}%\n\n")
            ...
            if coverage_metrics.get("platform_coverage"):
                ...
                for platform, coverage in coverage_metrics.get("platform_coverage", {}).items():
            ...
            f.write(f"- **Total CI Duration**: {performance_metrics.get('total_duration', 0):.2f} seconds\n\n")
            ...
            f.write(f"- **Total Vulnerabilities**: {security_metrics.get('total_vulnerabilities', 0)}\n")
            f.write(f"- **High Severity**: {security_metrics.get('high_vulnerabilities', 0)}\n")
            f.write(f"- **Medium Severity**: {security_metrics.get('medium_vulnerabilities', 0)}\n")
            f.write(f"- **Low Severity**: {security_metrics.get('low_vulnerabilities', 0)}\n\n")
            ...
            f.write(f"- **Total Platform Issues**: {platform_metrics.get('total_issues', 0)}\n")
            f.write(f"- **Error Level Issues**: {platform_metrics.get('severity_counts', {}).get('error', 0)}\n")
            f.write(f"- **Warning Level Issues**: {platform_metrics.get('severity_counts', {}).get('warning', 0)}\n")
            f.write(f"- **Info Level Issues**: {platform_metrics.get('severity_counts', {}).get('info', 0)}\n\n")
            ...
            if platform_metrics.get("platform_issues"):
                ...
                for platform, count in platform_metrics.get("platform_issues", {}).items():
    ...
    def generate_visualizations(self, history_data):
        ...
        timestamps = [entry.get("timestamp", "").split()[0] for entry in history_data]  # Just date part
        ...
        vulnerabilities = [entry.get("total_vulnerabilities", 0) for entry in history_data]
        ...
        axs[1, 1].plot(timestamps, vulnerabilities, marker='o', linestyle='-', color='red')
        ...
        axs[2, 0].plot(timestamps, platform_issues, marker='o', linestyle='-', color='purple')
    ...
    def generate_github_summary(self):
        ...
        with open(os.environ['GITHUB_STEP_SUMMARY'], 'a', encoding='utf-8') as f:
            ...
            # Platform coverage
            platform_coverage = self.metrics_data["coverage_metrics"].get("platform_coverage", {})
            ...
            slowest_steps = self.metrics_data["performance_metrics"].get("slowest_steps", [])
            if slowest_steps:
                f.write("Slowest steps:\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Collect CI metrics and generate reports")
    parser.add_argument("--artifacts-dir", default="ci_artifacts", help="Directory containing CI artifacts")
    parser.add_argument("--output-dir", default="ci_artifacts/metrics", help="Directory to store metrics reports")
