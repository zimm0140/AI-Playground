...
class PRCommentGenerator:
    ...
    def load_simulation_data(self) -> bool:
        ...
        try:
            simulation_file = os.path.join(self.simulation_dir, "workflow_simulation_results.json")
    ...
    def get_workflow_validation_status(self, filename: str) -> Dict[str, Any]:
        ...
        if not self.validation_data:
            return {"is_valid": False, "status": "unknown", "issues": ["No validation data available"]}
        ...
        return {"is_valid": False, "status": "unknown", "issues": ["Workflow not found in validation results"]}
    ...
    def get_workflow_requirements(self, filename: str) -> Dict[str, Any]:
        ...
        if not self.requirements_data:
            return {"is_analyzed": False, "status": "unknown", "memory_required": {"min": 0, "recommended": 0}}
        ...
        for workflow in self.requirements_data.get("workflows", []):
            if workflow.get("filename") == filename:
                return {
                    ...
                    "memory_required": workflow.get("memory_required", {"min": 0, "recommended": 0}),
    ...
    def get_workflow_test_status(self, filename: str) -> Dict[str, Any]:
        ...
        if not self.tests_data:
            return {"passed": False, "status": "unknown", "issues": ["No test data available"]}
        ...
        return {"passed": False, "status": "unknown", "issues": ["Workflow not found in test results"]}
    
    def get_workflow_simulation_status(self, filename: str) -> Dict[str, Any]:
        """Get simulation status for a specific workflow"""
        if not self.simulation_data:
            return {"success": False, "status": "unknown", "errors": ["No simulation data available"]}
        ...
        return {"success": False, "status": "unknown", "errors": ["Workflow not found in simulation results"]}
    ...
    def generate_comment(self) -> str:
        ...
        if failing_workflows:
            ...
            for workflow in failing_workflows:
                ...
                if workflow["simulation"]["status"] == "fail":
                    ...
                    for error in workflow["simulation"]["errors"][:5]:  # Show at most 5 errors
                        comment += f"- {error}\n"
                ...
                if workflow["changes"].get("node_count", 0) > 0:
                    ...
                    comment += f"- Total connections: {workflow['changes']['link_count']}\n"
        ...
        comment += "1. **Structural Validation**: Checks JSON structure and node connections\n"
        ...
        comment += "4. **Resource Analysis**: Estimates memory and dependency requirements\n"
...
def main():
    parser = argparse.ArgumentParser(description="Generate PR comment for ComfyUI workflow changes")
    ...
    parser.add_argument("--simulation-dir", default="ci_artifacts/workflow_simulation", help="Directory containing simulation results")
    parser.add_argument("--versions-dir", default="ci_artifacts/workflow_versions", help="Directory containing version history")
    parser.add_argument("--dashboard-dir", default="ci_artifacts/workflow_dashboard", help="Directory containing dashboard")
    parser.add_argument("--changed-files", nargs="+", help="List of changed files in the PR")
    parser.add_argument("--output-file", default="workflow_pr_comment.md", help="Output file for PR comment")
    ...
    if not changed_files:
        print("No changed files provided. Please provide --changed-files or set CHANGED_FILES environment variable")
        sys.exit(1)
