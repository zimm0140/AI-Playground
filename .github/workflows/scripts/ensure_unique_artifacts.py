...
def ensure_unique_artifacts():
    ...
    workflow_files = [f for f in workflow_files if not f.endswith("ensure_unique_artifacts.yml")]
    ...
    conflicts = {name: locations for name, locations in artifact_names.items() if len(locations) > 1}
    ...
    for file_path in workflow_files:
        ...
        workflow_name_match = re.search(r"name:\s*([^\n]+)", ''.join(lines))
        workflow_prefix = workflow_name_match.group(1).strip() if workflow_name_match else os.path.basename(file_path)
        workflow_prefix = workflow_prefix.lower().replace(' ', '-')[:10]  # Keep it short
            ...
            if in_upload_section:
                ...
                if artifact_match:
                    ...
                    if artifact_name in conflicts:
                        ...
                        job_prefix = (current_job_match or current_job or workflow_prefix).lower()
                        job_prefix = re.sub(r'[^a-z0-9-]', '', job_prefix.replace(' ', '-'))[:10]
                        
                        # Only update if the name isn't already unique
                        if not artifact_name.endswith(f"-{job_prefix}") and not artifact_name.startswith(f"{job_prefix}-"):
                            if artifact_name in ['build-artifacts', 'release-artifacts', 'test-results', 
                                                'coverage-data', 'pytest-results', 'tool-compatibility-report',
                                                'hardware-support-matrix', 'api-documentation']:
                            ...
                            print(f"  - Renamed '{artifact_name}' to '{new_name}' in {file_path}")
                            updated = True
                
                # Check for end of upload-artifact section
                if line.strip() and not line.strip().startswith('-') and not line.strip().startswith('name:') and not line.strip().startswith('path:') and not line.strip().startswith('retention-days:') and not line.strip().startswith('if-no-files-found:') and not line.strip().startswith('with:'):
                    in_upload_section = False
