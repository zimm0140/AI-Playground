...
class BadgesGenerator:
    def __init__(self, artifacts_dir="ci_artifacts", output_dir="ci_artifacts/badges"):
        ...
        # Badge details
        self.badges = {
            "build": {"label": "build", "message": "unknown", "color": self.colors["inactive"]},
            "coverage": {"label": "coverage", "message": "0%", "color": self.colors["inactive"]},
            "tests": {"label": "tests", "message": "unknown", "color": self.colors["inactive"]},
            "security": {"label": "security", "message": "unknown", "color": self.colors["inactive"]},
            "platforms": {"label": "platforms", "message": "unknown", "color": self.colors["inactive"]},
            "docs": {"label": "docs", "message": "unknown", "color": self.colors["inactive"]},
            "dependencies": {"label": "dependencies", "message": "unknown", "color": self.colors["inactive"]}
        }
        
        # Links to GitHub Actions workflows
        self.github_actions_url = os.environ.get("GITHUB_SERVER_URL", "https://github.com")
    ...
    def generate_build_badge(self):
        ...
        if build_result == "success":
            ...
            perf_files = glob.glob(f"{self.artifacts_dir}/performance*/performance_data.csv")
            if perf_files:
                self.badges["build"]["message"] = "passing"
                self.badges["build"]["color"] = self.colors["success"]
        ...
        if os.path.exists(metrics_file):
            try:
                ...
                if total_vulns == 0:
                    ...
                    self.badges["security"]["message"] = f"{total_vulns} vulnerabilities"
    ...
    def generate_platforms_badge(self):
        ...
        windows_report = glob.glob(f"{self.artifacts_dir}/compatibility-report-windows*")
    ...
    def generate_docs_badge(self):
        ...
        api_docs = glob.glob(f"{self.artifacts_dir}/api-documentation*") + \
                 glob.glob(f"{self.artifacts_dir}/api-endpoints-summary*")
    ...
    def generate_dependencies_badge(self):
        ...
        dependency_report = os.path.join(self.artifacts_dir, "dependencies/main-diff.txt")
        ...
        if os.path.exists(dependency_report):
            try:
                ...
                if outdated_count == 0:
                    ...
                    self.badges["dependencies"]["message"] = f"{outdated_count} outdated"
    ...
    def generate_github_summary(self):
        ...
        with open(os.environ['GITHUB_STEP_SUMMARY'], 'a', encoding='utf-8') as f:
            ...
            for badge_type, badge_config in self.badges.items():
                f.write(f"| {badge_config['label']} | {badge_config['message']} | {badge_config['color']} |\n")
...
def main():
    ...
    parser.add_argument("--output-dir", default="ci_artifacts/badges", help="Directory to store badges")
