...
class ComfyWorkflowDashboard:
    ...
    def load_validation_data(self) -> bool:
        ...
        try:
            validation_file = os.path.join(self.validation_dir, "workflow_validation_results.json")
            if os.path.exists(validation_file):
                with open(validation_file, 'r', encoding='utf-8') as f:
                    self.validation_data = json.load(f)
                print(f"Loaded validation data for {len(self.validation_data.get('workflows', []))} workflows")
                return True
            else:
    ...
    def load_requirements_data(self) -> bool:
        ...
        try:
            requirements_file = os.path.join(self.requirements_dir, "workflow_requirements_results.json")
            if os.path.exists(requirements_file):
                with open(requirements_file, 'r', encoding='utf-8') as f:
                    self.requirements_data = json.load(f)
                print(f"Loaded requirements data for {len(self.requirements_data.get('workflows', []))} workflows")
                return True
            else:
    ...
    def load_tests_data(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(tests_file):
                ...
                print(f"Loaded test data for {len(self.tests_data.get('workflow_results', []))} workflows")
                return True
            else:
    ...
    def load_versions_data(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(history_file):
                ...
                print(f"Loaded version history for {len(self.versions_data.get('workflows', []))} workflows")
                return True
            else:
    ...
    def integrate_data(self) -> Dict[str, Dict[str, Any]]:
            ...
            for workflow in self.validation_data.get("workflows", []):
                ...
                if filename in self.dashboard_data:
                    ...
                    if workflow.get("is_valid", False):
                        ...
                        has_errors = any(issue.get("type", "").endswith("_error") for issue in workflow.get("issues", []))
        ...
        if self.requirements_data:
            for workflow in self.requirements_data.get("workflows", []):
                ...
                if filename in self.dashboard_data:
                    ...
                    self.dashboard_data[filename]["requirements"]["memory_required"] = workflow.get("memory_required", {"min": 0, "recommended": 0})
                    
                    # Set requirements status based on analysis success
                    self.dashboard_data[filename]["status"]["requirements"] = "pass" if workflow.get("is_analyzed", False) else "fail"
        ...
        if self.tests_data:
            for workflow in self.tests_data.get("workflow_results", []):
                ...
                filename = workflow["filename"]
                if filename in self.dashboard_data:
                    self.dashboard_data[filename]["tests"]["passed"] = workflow.get("passed", False)
                    self.dashboard_data[filename]["tests"]["issues"] = workflow.get("issues", [])
        ...
        if self.versions_data:
            for workflow in self.versions_data.get("workflows", []):
                ...
                if filename in self.dashboard_data:
                    ...
                    self.dashboard_data[filename]["versions"]["total_versions"] = len(versions)
                    ...
                    if versions:
                        ...
                        has_breaking = any(change.startswith("Removed") or "type changed" in change 
    ...
    def generate_dashboard_markdown(self) -> str:
        ...
        try:
            with open(md_path, 'w', encoding='utf-8') as f:
                ...
                f.write("| Workflow | Validation | Test | Memory Req. | Nodes | Custom Nodes | Versions | Status |\n")
                f.write("|----------|------------|------|-------------|-------|--------------|----------|--------|\n")
                ...
                for filename, data in sorted(self.dashboard_data.items()):
                    ...
                    if versions["latest_version"]:
                        ...
                        for workflow in self.versions_data.get("workflows", []):
                            if workflow.get("filename") == filename:
                                latest_version = max(workflow.get("versions", []), key=lambda v: v.get("timestamp", ""))
                    ...
                    f.write(f"| {filename} | {validation_status} | {test_status} | {memory_text} | {node_count} | {custom_nodes_text} | {version_status} | {status_text} |\n")
                ...
                f.write("- ⚠️ **Warning**: Minor issues that might not affect functionality\n")
                f.write("- ❌ **Fail**: Critical issues that need to be addressed\n")
                f.write("- ❓ **Unknown**: Not enough information to determine status\n\n")
                ...
                for filename, data in self.dashboard_data.items():
                    grouped_workflows[data["status"]["overall"]].append((filename, data))
                ...
                if grouped_workflows["fail"]:
                    ...
                    for filename, data in sorted(grouped_workflows["fail"]):
                        ...
                        f.write(f"- Memory: Min {data['requirements']['memory_required']['min']}GB, Recommended {data['requirements']['memory_required']['recommended']}GB\n")
                        if data["requirements"]["custom_nodes"]:
                            f.write(f"- Custom Nodes: {', '.join(data['requirements']['custom_nodes'])}\n")
                        f.write("\n")
                ...
                if grouped_workflows["warning"]:
                    ...
                    for filename, data in sorted(grouped_workflows["warning"]):
                        ...
                        warnings = [issue for issue in data["validation"]["issues"] 
                        ...
                        # Test warnings
                        warnings = [issue for issue in data["tests"]["issues"] 
                ...
                if grouped_workflows["fail"]:
                    ...
                    for filename, data in sorted(grouped_workflows["fail"]):
                        ...
                        if [issue for issue in data["tests"]["issues"] if issue.startswith("Error:")]:
                ...
                if grouped_workflows["warning"]:
                    ...
                    for filename, data in sorted(grouped_workflows["warning"]):
                        ...
                        if [issue for issue in data["tests"]["issues"] if not issue.startswith("Error:")]:
                ...
                # Final notes
                f.write("## Notes\n\n")
                f.write("1. This dashboard is automatically generated by the CI workflow.\n")
                f.write("2. Workflows with high memory requirements may not run on all systems.\n")
                f.write("3. Breaking changes in workflows may affect compatibility with older versions.\n")
                f.write("4. Custom nodes required by workflows must be installed separately.\n\n")
        ...
        try:
            with open(os.environ['GITHUB_STEP_SUMMARY'], 'a', encoding='utf-8') as f:
                ...
                if status_counts["fail"] > 0:
                    f.write(f"❌ **{status_counts['fail']} workflow(s) have critical issues**\n\n")
                elif status_counts["warning"] > 0:
                    f.write(f"⚠️ **{status_counts['warning']} workflow(s) have warnings**\n\n")
                else:
                ...
                failing_workflows = [(filename, data) for filename, data in self.dashboard_data.items() 
                ...
                if failing_workflows:
                    ...
                    for filename, data in sorted(failing_workflows):
                        ...
                        issues_text = ", ".join(critical_issues[:2])  # Show only first 2 issues
                        if len(critical_issues) > 2:
                            issues_text += f", +{len(critical_issues) - 2} more"
                        
                        f.write(f"| {filename} | {validation_status} | {test_status} | {issues_text} |\n")
...
def main():
    ...
    parser.add_argument("--validation-dir", default="ci_artifacts/workflow_validation", help="Directory containing validation results")
    parser.add_argument("--requirements-dir", default="ci_artifacts/workflow_requirements", help="Directory containing requirements analysis")
    parser.add_argument("--tests-dir", default="ci_artifacts/workflow_tests", help="Directory containing test results")
    parser.add_argument("--versions-dir", default="ci_artifacts/workflow_versions", help="Directory containing version history")
    parser.add_argument("--output-dir", default="ci_artifacts/workflow_dashboard", help="Directory to store dashboard")
