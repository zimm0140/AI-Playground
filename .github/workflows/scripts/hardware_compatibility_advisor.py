...
class HardwareCompatibilityAdvisor:
    ...
    def analyze_conflicts(self) -> Dict[str, Any]:
        ...
        for package, conflict in conflicts_by_package.items():
            ...
            all_constraints = all(any(op in v for op in ['>=', '<=', '>', '<', '~=']) for v in versions)
    ...
    def generate_resolution_plan(self) -> Dict[str, Dict[str, str]]:
        ...
        for recommendation in self.recommendations.get('high_priority', []):
            ...
            for hw_name in recommendation['affected_platforms']:
                for file_name in hw_requirements.get(hw_name, {}):
                    if hw_name in resolution_plans and file_name in resolution_plans[hw_name]:
                        if package in resolution_plans[hw_name][file_name]:
                            # Update the version
                            resolution_plans[hw_name][file_name][package] = suggested_version
        ...
        for recommendation in self.recommendations.get('medium_priority', []):
            ...
            high_priority_packages = [r['package'] for r in self.recommendations.get('high_priority', [])]
    ...
    def write_resolution_plan(self) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## Overview\n\n")
            f.write("This plan provides specific recommendations for resolving package compatibility issues across hardware platforms.\n\n")
            ...
            if high_priority_count == 0:
                ...
                for recommendation in self.recommendations.get('high_priority', []):
                    package = recommendation['package']
                    current_versions = ', '.join(f"`{v}`" for v in recommendation['current_versions'])
                    suggested_version = recommendation['suggested_version']
            ...
            f.write("The following sections provide standardized requirements files for each platform with resolved conflicts.\n\n")
    ...
    def generate_patch_files(self) -> List[str]:
        ...
        for hw_name, hw_files in self.optimized_requirements.items():
            for file_name, packages in hw_files.items():
                ...
                patch_file = os.path.join(self.output_dir, f"{hw_name}_{file_name}.patch")
    ...
    def write_github_summary(self) -> str:
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Generated **{total_recommendations}** recommendations for resolving hardware compatibility issues:\n\n")
            
            f.write("| Priority | Count | Action Required |\n")
            f.write("|----------|-------|----------------|\n")
            f.write(f"| 🔴 High | {high_priority_count} | Immediate attention required |\n")
            f.write(f"| 🟡 Medium | {medium_priority_count} | Review in next development cycle |\n")
            ...
            if high_priority_count > 0:
                ...
                for recommendation in self.recommendations.get('high_priority', [])[:3]:  # Show up to 3
                    package = recommendation['package']
                    suggested_version = recommendation['suggested_version']
                    affected_platforms = ', '.join(recommendation['affected_platforms'])
                    
                    f.write(f"- **{package}**: Standardize on `{suggested_version}` across {affected_platforms}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze hardware compatibility and provide recommendations")
    ...
    parser.add_argument("--output-dir", default="ci_artifacts/hardware_compatibility/recommendations",
