...
class HardwareCompatibilityAutofix:
    ...
    def load_recommendations(self) -> bool:
        ...
        try:
            ...
            high_priority_match = re.search(r'## High Priority Recommendations\s*\n\n(.*?)(?=\n\n##|\Z)', 
                                           content, re.DOTALL)
            
            if high_priority_match:
                high_priority_text = high_priority_match.group(1)
                # Parse the recommendations
                self.resolution_plan['high_priority'] = self._parse_recommendations(high_priority_text)
            else:
                self.resolution_plan['high_priority'] = []
            
            # If we want medium priority recommendations too
            if not self.high_priority_only:
                medium_priority_match = re.search(r'## Medium Priority Recommendations\s*\n\n(.*?)(?=\n\n##|\Z)', 
                                               content, re.DOTALL)
                
                if medium_priority_match:
                    medium_priority_text = medium_priority_match.group(1)
                    # Parse the recommendations
                    self.resolution_plan['medium_priority'] = self._parse_recommendations(medium_priority_text)
                else:
    ...
    def _parse_recommendations(self, text: str) -> List[Dict[str, str]]:
        ...
        package_sections = re.split(r'### ([\w-]+)\s*\n', text)[1:]  # Skip the first empty element
        ...
        for i in range(0, len(package_sections), 2):
            if i + 1 < len(package_sections):
                ...
                suggested_version_match = re.search(r'\*\*Suggested Version\*\*: `(.*?)`', package_content)
                if suggested_version_match:
                    suggested_version = suggested_version_match.group(1)
                    
                    # Extract affected platforms
                    affected_platforms_match = re.search(r'\*\*Affected Platforms\*\*: (.*?)\s*$', 
                                                      package_content, re.MULTILINE)
                    affected_platforms = []
                    if affected_platforms_match:
                        affected_platforms = [p.strip() for p in affected_platforms_match.group(1).split(',')]
            ...
            for hw_name in affected_platforms:
                for file_name, file_data in hw_requirements.get(hw_name, {}).items():
                    if package in file_data:
                        ...
                        original_file = self._find_original_file_path(hw_name, file_name)
                        ...
                        try:
                            self._update_package_version(original_file, package, suggested_version)
    ...
    def _find_original_file_path(self, hw_name: str, file_name: str) -> str:
        ...
        """
        for file_path in self.compatibility_data.get('hardware_requirements', {}).get(hw_name, {}).keys():
            if os.path.basename(file_path) == file_name:
                return file_path
        
        return ""
    
    def _update_package_version(self, file_path: str, package: str, version: str) -> bool:
        """
    ...
    def generate_report(self) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            total_applied = sum(len(changes) for changes in self.applied_changes.values())
            total_skipped = sum(len(changes) for changes in self.skipped_changes.values())
            
            f.write(f"- **Mode**: {'High Priority Only' if self.high_priority_only else 'All Recommendations'}\n")
            f.write(f"- **Dry Run**: {'Yes (no changes applied)' if self.dry_run else 'No (changes applied)'}\n")
            ...
            if total_applied == 0:
                ...
                for hw_name, changes in sorted(self.applied_changes.items()):
                    ...
                    for change in changes:
                        ...
                        f.write(f"| {package} | {file_name} | `{original_version}` | `{new_version}` |\n")
            ...
            if total_skipped == 0:
                ...
                for hw_name, changes in sorted(self.skipped_changes.items()):
                    ...
                    for change in changes:
                        package = change['package']
                        file_name = os.path.basename(change['file']) if isinstance(change['file'], str) else change['file']
            ...
            if self.dry_run:
                f.write("This was a dry run, so no changes were actually applied. To apply the changes, run with `--no-dry-run`.\n\n")
            else:
                f.write("1. Review the applied changes to ensure they meet your requirements.\n")
                f.write("2. If needed, restore from backups (files with `.bak` extension).\n")
                f.write("3. Run the hardware compatibility tests again to verify that conflicts have been resolved.\n\n")
    ...
    def write_github_summary(self) -> str:
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"**Mode**: {'🔴 High Priority Only' if self.high_priority_only else '🟠 All Recommendations'} | ")
            f.write(f"**Run Type**: {'🔍 Dry Run (no changes)' if self.dry_run else '🛠️ Live Run (applied changes)'}\n\n")
            
            # Summary counts
            f.write("| Result | Count | Details |\n")
            f.write("|--------|-------|--------|\n")
            f.write(f"| ✅ Applied | {total_applied} | Changes successfully applied |\n")
            f.write(f"| ⚠️ Skipped | {total_skipped} | Changes that could not be applied |\n\n")
            ...
            if total_applied > 0:
                ...
                for sample in samples[:5]:
                    f.write(f"- {sample['hardware']}: `{sample['package']}` from `{sample['from']}` to `{sample['to']}`\n")
...
def main():
    parser = argparse.ArgumentParser(description="Auto-fix hardware compatibility issues")
    parser.add_argument("--input-dir", default="ci_artifacts/hardware_compatibility",
                      help="Directory containing hardware compatibility test results")
    parser.add_argument("--recommendations-dir", default="ci_artifacts/hardware_compatibility/recommendations",
                      help="Directory containing advisor recommendations")
    parser.add_argument("--output-dir", default="ci_artifacts/hardware_compatibility/autofix",
                      help="Directory to store auto-fix results")
    parser.add_argument("--resolution-plan", default="resolution_plan.md",
                      help="Markdown file with resolution plan")
    parser.add_argument("--data-file", default="hardware_compatibility_data.json",
                      help="JSON file with compatibility test data")
    parser.add_argument("--backup-suffix", default=".bak",
                      help="Suffix for backup files")
    parser.add_argument("--high-priority-only", action="store_true", default=True,
    ...
    high_priority_only = not args.all_priorities if args.all_priorities else args.high_priority_only
