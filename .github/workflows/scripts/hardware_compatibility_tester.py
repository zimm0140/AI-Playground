...
class HardwareCompatibilityTester:
    ...
    def find_hardware_requirements(self) -> Dict[str, List[str]]:
        ...
        default_req = [f for f in glob.glob("**/requirements.txt", recursive=True) 
                      if not any(re.match(pattern, f, re.IGNORECASE) 
        ...
        for package in all_packages:
            ...
            if len(all_versions) > 1:
                ...
                conflicts.append({
                    ...
                    'severity': 'high' if '==' in ''.join(all_versions) else 'medium'
                })
    ...
    def generate_markdown_report(self) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
                ...
                for hw2 in hw_platforms:
                    if hw1 == hw2:
                        ...
                        if hw2 in self.hardware_compatibility_matrix[hw1]:
                            f.write(f" {self.hardware_compatibility_matrix[hw1][hw2]['score']} |")
                        else:
                            f.write(" N/A |")
                f.write("\n")
            ...
            if not self.conflict_data:
                ...
                f.write(f"Found {len(self.conflict_data)} packages with compatibility issues.\n\n")
            ...
            if not self.conflict_data:
                f.write("All packages are compatible across hardware platforms. No action required.\n")
            else:
                f.write("### High Priority Fixes\n\n")
                
                high_priority = [c for c in self.conflict_data if c['severity'] == 'high']
                if high_priority:
                    for conflict in high_priority:
                        package = conflict['package']
                        versions = ", ".join(f"`{v}`" for v in sorted(conflict['all_versions']))
                        f.write(f"- **{package}**: Standardize version across platforms. Current versions: {versions}\n")
                else:
                    f.write("No high priority fixes required.\n")
                
                f.write("\n### Medium Priority Fixes\n\n")
                
                medium_priority = [c for c in self.conflict_data if c['severity'] == 'medium']
                if medium_priority:
                    for conflict in medium_priority:
                        package = conflict['package']
                        versions = ", ".join(f"`{v}`" for v in sorted(conflict['all_versions']))
                        f.write(f"- **{package}**: Consider standardizing version requirements. Current specs: {versions}\n")
                else:
    ...
    def generate_github_summary(self) -> None:
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            platforms_list = ", ".join(f"`{hw}`" for hw in sorted(self.hardware_requirements.keys()))
            f.write(f"{platforms_list}\n\n")
            
            # Show conflict summary
            conflict_count = len(self.conflict_data)
            high_priority = len([c for c in self.conflict_data if c['severity'] == 'high'])
            ...
            if conflict_count > 0:
                ...
                if high_priority > 0:
                    ...
                    for conflict in [c for c in self.conflict_data if c['severity'] == 'high']:
                        package = conflict['package']
            ...
            total_pairs = sum(1 for hw1 in self.hardware_compatibility_matrix 
                             for hw2 in self.hardware_compatibility_matrix[hw1] if hw1 != hw2)
            
            fully_compatible = sum(1 for hw1 in self.hardware_compatibility_matrix 
                                  for hw2 in self.hardware_compatibility_matrix[hw1] 
                                  if hw1 != hw2 and self.hardware_compatibility_matrix[hw1][hw2]['score'] == "100% Compatible")
            
            compatibility_rate = fully_compatible / total_pairs * 100 if total_pairs > 0 else 0
            
            f.write(f"- **Overall compatibility rate**: {compatibility_rate:.1f}%\n")
            f.write(f"- **Fully compatible hardware pairs**: {fully_compatible} out of {total_pairs}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Analyze hardware-specific package compatibility")
    parser.add_argument("--output-dir", default="ci_artifacts/hardware_compatibility",
                       help="Directory to store output reports")
    parser.add_argument("--req-files-pattern", default="**/*requirements*.txt",
    ...
    tester = HardwareCompatibilityTester(
        ...
        req_files_pattern=args.req_files_pattern
    )
