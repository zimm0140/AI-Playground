    ...
    for file_path in file_paths:
        ...
        try:
            ...
            for line_num, issue in not_in_issues:
                print(f"  ⚠️ Consider using 'not in' instead of '{issue}' at line {line_num}")
                error_count += 1
        ...
        if stripped == 'try:':
            ...
            if i + 1 < len(lines):
                ...
                if next_stripped and not next_line.startswith(' ' * (indent + 4)):
                    issues.append((i+2, f"Expected indentation of {indent + 4} spaces after 'try:'"))
            ...
            if indent != try_indent:
                issues.append((i+1, f"'except' block should have same indentation as 'try'"))
            in_try_block = False
...
if __name__ == "__main__":
    ...
    if os.path.basename(script_dir) == 'scripts' and os.path.exists(os.path.join(os.path.dirname(script_dir), 'workflows')):
