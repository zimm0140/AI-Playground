...
class NotificationSender:
    def __init__(self, metrics_file=None, artifacts_dir="ci_artifacts"):
        ...
        self.metrics_file = metrics_file or os.path.join(artifacts_dir, "metrics/ci_metrics.json")
        ...
        self.run_url = f"{self.github_server}/{self.github_repo}/actions/runs/{self.github_run_id}"
        self.commit_url = f"{self.github_server}/{self.github_repo}/commit/{self.github_sha}"
    ...
    def prepare_notification_content(self, status="success"):
        ...
        if status == "success":
            self.notification_title = f"✅ CI Succeeded: {self.github_workflow} #{self.github_run_number}"
        else:
            self.notification_title = f"❌ CI Failed: {self.github_workflow} #{self.github_run_number}"
...
View Details: {self.run_url}
    ...
    def send_email(self, recipients, smtp_server, smtp_port=587, smtp_user=None, smtp_password=None, sender=None):
                ...
                "sections": [
                    {
                        ...
                        "facts": [
                            ...
                            {
                                "name": "Run",
                                "value": f"#{self.github_run_number}"
                            },
                            {
...
def main():
    ...
    parser.add_argument("--status", default="success", choices=["success", "failure"], help="CI run status")
    parser.add_argument("--notify-success", action="store_true", help="Send notifications on success (default: only on failure)")
