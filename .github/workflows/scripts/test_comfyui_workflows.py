...
def parse_args():
    ...
    parser.add_argument("--output-dir", required=True, help="Directory to write test results")
...
def main():
    ...
    if os.path.isdir(args.workflows_dir):
        workflow_files = [
            ...
            if f.endswith('.json') and os.path.isfile(os.path.join(args.workflows_dir, f))
        ]
    ...
    for file_path in workflow_files:
        ...
        status = "✅ Passed" if result["test_passed"] else f"❌ Failed ({len(result['errors'])} errors)"
