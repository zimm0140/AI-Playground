...
class ComfyWorkflowTester:
    ...
    def __init__(self, workflows_dir, output_dir="ci_artifacts/workflow_tests"):
        ...
        self.node_types = {
            ...
            "LoraLoader": {
                "inputs": {"model": "model", "clip": "clip_model", "lora_name": "string", "strength": "float"},
            ...
            "UpscaleImage": {
                "inputs": {"image": "image", "upscale_method": "string", "scale": "float"},
                "outputs": {"image": "image"}
            },
        ...
        for node in graph:
            if node not in visited:
                if has_cycle(node, visited, path):
                    return True, f"Circular dependency detected starting from node {node}"
    ...
    def simulate_execution(self, workflow):
            ...
            for input_name in node_def["inputs"]:
                ...
                if link_key in links_map:
                    ...
                    if from_node not in node_outputs:
                        issues.append(f"Error: Source node {from_node} not processed before {node_id}")
                        continue
                        
                    # Check if source node has the output
                    if from_slot not in node_outputs[from_node]:
                        issues.append(f"Error: Source node {from_node} has no output {from_slot}")
                        continue
                    ...
                    if input_type and output_type and not self.check_type_compatibility(output_type, input_type):
                        issues.append(f"Warning: Type mismatch: {from_node}:{from_slot} ({output_type}) -> {node_id}:{input_name} ({input_type})")
        ...
        for node_id, node_data in workflow["nodes"].items():
            if "class_type" in node_data and node_data["class_type"] in output_node_types:
        ...
        success = len(issues) == 0 or all(issue.startswith("Warning") for issue in issues)
    ...
    def test_all_workflows(self):
        ...
        for file_path in workflow_files:
            ...
            if result["passed"]:
                ...
                logger.info(f"✅ Passed: {result['filename']} ({result['execution_time']:.2f}s)")
            else:
                self.results["summary"]["failed_workflows"] += 1
                logger.info(f"❌ Failed: {result['filename']} ({result['execution_time']:.2f}s)")
    ...
    def generate_markdown_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            # Summary section
            f.write("## Summary\n\n")
            f.write(f"- Total workflows tested: {self.results['summary']['total_workflows']}\n")
            ...
            for result in sorted_results:
                ...
                f.write(f"| {result['filename']} | {status} | {result['execution_time']:.2f}s | {issues_text} |\n")
            
            # Failed workflow details
            failed_results = [r for r in self.results["workflow_results"] if not r["passed"]]
            ...
            f.write("1. **Fix Circular Dependencies**: Workflows should not contain circular references between nodes.\n")
            f.write("2. **Connect Output Nodes**: Ensure workflows have properly connected output nodes (SaveImage or PreviewImage).\n")
            f.write("3. **Fix Type Mismatches**: Ensure connections between nodes use compatible types.\n")
            f.write("4. **Update Node Definitions**: If the test reports unknown node types, they may need to be added to the test definitions.\n\n")
            
            f.write("---\n")
            f.write("*This report was automatically generated by the ComfyUI workflow test script.*\n")
        ...
        with open(os.environ['GITHUB_STEP_SUMMARY'], 'a', encoding='utf-8') as f:
            ...
            if self.results["summary"]["failed_workflows"] > 0:
                ...
                for result in failed_results[:10]:  # Show at most 10 failures
                    error_count = sum(1 for issue in result["issues"] if issue.startswith("Error"))
                    warning_count = len(result["issues"]) - error_count
                    
                    f.write(f"- **{result['filename']}**: {error_count} errors, {warning_count} warnings\n")
...
def main():
    ...
    parser.add_argument("--output-dir", default="ci_artifacts/workflow_tests", help="Directory to store test results")
    parser.add_argument("--fail-on-error", action="store_true", help="Exit with error if any workflows fail tests")
