...
def update_metrics(metrics, step_name=None, step_duration=None, status="running"):
    ...
    if status in ["success", "failure", "cancelled", "completed"]:
        ...
        metrics["duration"]["total_seconds"] = metrics["duration"]["end_time"] - metrics["duration"]["start_time"]
...
def save_metrics(metrics, output_dir):
    ...
    summary_path = os.path.join(output_dir, f"ci_metrics_summary_{workflow_name}_{job_name}.md")
...
def write_summary_markdown(metrics, output_path):
    ...
    with open(output_path, 'w') as f:
        f.write(f"# CI Performance Metrics\n\n")
        f.write(f"## Job Information\n")
        ...
        # Cache stats
        f.write(f"## Cache Performance\n")
        total_cache_requests = metrics['cache']['hits'] + metrics['cache']['misses']
        hit_rate = metrics['cache']['hits'] / total_cache_requests * 100 if total_cache_requests > 0 else 0
        ...
        # Step performance
        if metrics['steps']:
            f.write(f"## Step Performance\n")
            f.write(f"| Step | Duration | Status |\n")
            f.write(f"|------|----------|--------|\n")
...
def parse_args():
    ...
    parser.add_argument(
        "--step",
        help="Name of the step being tracked",
        default=None
    )
    ...
    parser.add_argument(
        "--init",
        help="Initialize metrics collection",
        action="store_true"
    )
    
    parser.add_argument(
        "--finalize",
        help="Finalize metrics collection",
        action="store_true"
    )
...
def main():
    """Main function"""
    args = parse_args()
    metrics_file = os.path.join(args.output_dir, f"current_metrics_{args.workflow}_{args.job}.json")
