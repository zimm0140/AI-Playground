...
class WorkflowHistory:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "WorkflowHistory":
        ...
        history.versions = [
            WorkflowVersion.from_dict(v) for v in data.get("versions", [])
        ]
        return history
...
class WorkflowVersionTracker:
                ...
                json.dump(
                    {
                        ...
                        "workflows": [
                            h.to_dict() for h in self.workflow_history.values()
                        ],
                    },
                    f,
                    indent=2,
    ...
    def detect_changes(self, old_workflow: dict, new_workflow: dict) -> list[str]:
        ...
        if added_nodes:
            node_types = [
                new_nodes_dict[node_id].get("class_type", "unknown")
        ...
        if removed_nodes:
            node_types = [
                old_nodes_dict[node_id].get("class_type", "unknown")
        ...
        old_link_tuples = [
            tuple(link[:4]) if len(link) >= 4 else tuple(link) for link in old_links
        ]
        new_link_tuples = [
            tuple(link[:4]) if len(link) >= 4 else tuple(link) for link in new_links
        ]
        ...
        try:
            ...
            if latest is None:
                ...
                if os.path.exists(file_path + ".prev"):
                    with open(file_path + ".prev", encoding="utf-8") as f:
    ...
    def generate_version_report(self) -> str:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            total_versions = sum(
                len(h.versions) for h in self.workflow_history.values()
            )
    ...
    def generate_compatibility_matrix(self) -> str:
        ...
        hw_configs = {
            "minimum": {"description": "Minimum Requirements (8GB GPU)", "memory": 8},
            "recommended": {
                "description": "Recommended Requirements (12GB GPU)",
                "memory": 12,
            },
            "high_end": {"description": "High-End System (24GB+ GPU)", "memory": 24},
        }
...
def main():
    ...
    parser.add_argument(
        "--history-file",
        default=None,
        help="File to store version history (default: ci_artifacts/workflow_versions/workflow_history.json)",
    )
    parser.add_argument(
        "--output-dir",
