#!/bin/bash
# Script to update all instances of actions/cache from v3 to v4
...
echo -e "${GREEN}Updating GitHub Action references from cache@v3 to v4${NC}"
...
for file in $WORKFLOW_FILES; do
    ...
    if grep -q "actions/cache@v3" "$file"; then
        ...
        if grep -q "actions/cache@v4" "$file"; then
