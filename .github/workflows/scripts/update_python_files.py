...
def fix_ruff_issues(files):
    ...
    for filename in files:
        ...
        # Fix imports
        run_command(
            ["ruff", "check", "--select=F401,F403,F405"] + ruff_common_args[1:] + ["--fix", filename],
            show_output=False
        )
        
        # Fix formatting
        run_command(
            ["ruff", "check", "--select=E"] + ruff_common_args[1:] + ["--fix", filename],
            show_output=False
        )
        
        # Fix other issues
        run_command(
            ["ruff", "check"] + ruff_common_args + ["--fix", filename],
            show_output=True
        )
