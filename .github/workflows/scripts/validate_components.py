...
def validate_component(component_file):
    ...
    required_fields = ["name", "description", "version", "type", "inputs", "outputs", "nodes"]
    ...
    if "version" in component:
        ...
        if len(parts) != 3 or not all(part.isdigit() for part in parts):
            issues.append(f"Invalid version format: {version}. Should be semver (e.g., 1.0.0)")
        ...
        for i, input_item in enumerate(component["inputs"]):
            ...
            for field in input_required_fields:
                if field not in input_item:
                    issues.append(f"Input {i} ({input_item.get('name', 'unnamed')}) missing required field: {field}")
        ...
        for i, output_item in enumerate(component["outputs"]):
            ...
            for field in output_required_fields:
                if field not in output_item:
                    issues.append(f"Output {i} ({output_item.get('name', 'unnamed')}) missing required field: {field}")
        ...
        if not isinstance(component["inputMappings"], dict):
            ...
            for input_name, mapping in component["inputMappings"].items():
                ...
                if "nodeId" not in mapping:
                    issues.append(f"Input mapping for {input_name} missing required field: nodeId")
                    
                if "inputName" not in mapping:
                    issues.append(f"Input mapping for {input_name} missing required field: inputName")
    ...
    if "outputMappings" in component:
        if not isinstance(component["outputMappings"], dict):
            ...
            for output_name, mapping in component["outputMappings"].items():
                ...
                if "nodeId" not in mapping:
                    issues.append(f"Output mapping for {output_name} missing required field: nodeId")
                    
                if "outputIndex" not in mapping:
                    issues.append(f"Output mapping for {output_name} missing required field: outputIndex")
...
def main():
    ...
    parser.add_argument("--components-dir", required=True, help="Directory containing component JSON files")
