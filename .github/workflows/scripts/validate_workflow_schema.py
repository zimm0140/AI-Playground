...
def validate_workflow(workflow_file, schema):
    ...
    try:
        ...
        return False, f"Schema validation error: {e.message} at {'/'.join([str(p) for p in e.path])}"
...
def main():
    parser = argparse.ArgumentParser(description="Validate ComfyUI workflow JSON files against schema")
    ...
    parser.add_argument("--output-file", required=True, help="Path to write the validation report")
