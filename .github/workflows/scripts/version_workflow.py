...
def detect_breaking_changes(old_workflow, new_workflow):
    ...
    old_inputs = {inp.get("nodeInput", ""): inp for inp in old_workflow.get("inputs", [])}
    new_inputs = {inp.get("nodeInput", ""): inp for inp in new_workflow.get("inputs", [])}
    ...
    for input_name in set(old_inputs.keys()) & set(new_inputs.keys()):
        ...
        if old_type != new_type:
            breaking_changes.append(f"Changed type of input '{input_name}' from '{old_type}' to '{new_type}'")
    
    # Check for changed model requirements
    old_models = {model.get("model", "") for model in 
                  old_workflow.get("comfyUIRequirements", {}).get("requiredModels", [])}
    new_models = {model.get("model", "") for model in 
                  new_workflow.get("comfyUIRequirements", {}).get("requiredModels", [])}
    
    removed_models = old_models - new_models
    if removed_models:
        breaking_changes.append(f"Removed model requirements: {', '.join(removed_models)}")
    
    # Check for changed backend
    if old_workflow.get("backend") != new_workflow.get("backend"):
        breaking_changes.append(f"Changed backend from '{old_workflow.get('backend')}' to '{new_workflow.get('backend')}'")
    ...
    for output_name in set(old_outputs.keys()) & set(new_outputs.keys()):
        ...
        if old_type != new_type:
            breaking_changes.append(f"Changed type of output '{output_name}' from '{old_type}' to '{new_type}'")
...
def find_previous_version(workflow_file):
    ...
    print("Warning: Finding previous versions requires integration with version control.")
...
def main():
    parser = argparse.ArgumentParser(description="Manage versioning for ComfyUI workflow files")
