...
def fix_table_of_contents(content: str) -> Tuple[str, List[str]]:
    ...
    for section in toc_sections:
        ...
        changes.append("Deduplicated and fixed numbering in Table of Contents")
    ...
    for section in toc_sections[1:]:
        ...
        changes.append("Removed duplicate Table of Contents section")
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Don't write changes, just report them")
    parser.add_argument("--exclude", "-e", help="Exclude pattern (e.g., 'node_modules')")
    parser.add_argument(
        "--fix-all",
        "-a",
        action="store_true",
